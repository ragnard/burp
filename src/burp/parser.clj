...
(defrecord CodeBlock [code children])
(defrecord InlineCode [code])
...
(def code              (bind [_    (token* "=")
                              code (optional text)]
                             (return {:children [(map->InlineCode
                                                  {:code (string/trim code)})]})))

(def inline            (bind [close?  (optional (token* "/"))
                              content (optional inline-content)]
                             (return {:self-close? (boolean close?)
                                      :inline-content (string/trim content)})))
...
(def tag               (bind [tag     tag-definition
                             ...
                             (return (map->Element
                                      (merge tag
                                             content
                                             {:attributes (into {} attr)})))))

(def code-tag          (bind [tag  tag-definition
                              attr (optional attributes)
                              code code]
                             (return (map->Element
                                      (merge tag
...
;;--------------------------------------------------------------------
;; Code Block
(def code-block        (bind [_    (token* "-")
                              code text]
                             (return (map->CodeBlock
                                      {:code (string/trim code)}))))
